...
class Companies_model extends MY_Model {
	...
	function lists($config) {
        $old_config = array();
        $aColumns = array('company_id','name','phone','address','contact_person_name');
        $wColumns = array ('name','phone','address','contact_person_name');
        ...
        foreach ($rResult->result_array() as $aRow) {
            ...
            $edit_url = site_url('company/edit/'.$aRow[$this->primaryKey].'/'.auth_key('company_edit'.$aRow[$this->primaryKey]));
            $edit_url .= '/'.$paging;
			$delete_url = site_url('company/delete/'.$aRow[$this->primaryKey]);

            $row[] = '<a  title="Edit" class="with-tip" href="'.$edit_url.'"><img src="'.base_url().'assets/images/icons/fugue/pencil.png" width="16" height="16" /></a>'
                     .'&nbsp;<a auth="'.auth_key('company_delete'.$aRow[$this->primaryKey]).'" title="Delete" class="with-tip ajaxdelete" href="'.$delete_url.'"><img src="'.base_url().'assets/images/icons/fugue/cross-circle.png" width="16" height="16" /></a>';
                     ;
